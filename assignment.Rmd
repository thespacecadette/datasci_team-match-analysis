...
seasonList <- list()
n_matches <- 5 # nrow(c_data$Season)
  ...
  for(i in 1:1) {
    ...
    seasonSubset <- data.frame(matrix(nrow=0,ncol=8))
    colnames(seasonSubset) <- c("no.games","no.wins","win.percentage","points.scored","points.conceded","total.snitches","goals","tournament.wins")
    seasonExistsIndex <- 0
    ...
    # insert/update in season
    
    # insert season subset back into season
    season_data <- list(season, seasonSubset)
    if(seasonExists) {
      seasonList[seasonExistsIndex] <- insert
    }else {
      append(seasonList, season_data, after = 0)
    }
  }
