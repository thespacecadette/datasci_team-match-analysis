...
```{r}
competitionResults <- read.csv("competitionResults.csv")

      # pointsCalculator = aggregate(cf$Number.of.cylinders, list(cf$Make), FUN=mean)
...
homeGameCalculations <- function (cf) {
  # numeric vars
  n_matches = nrow(cf$Season)
  # data vars
  team.statistics <- data.frame(matrix(numcol=3, numrow=0))
  ...
  for(i in 1:n_matches) {
    ...
    # Return: Count of 
    if(length(homeTeam.idx) != 0) {
      if(match$round == homeTeam) {
        # The number of HOME games each team has had
        totalHomeGames <- team.statistics[homeTeam.idx, 3] + 1
        team.statistics[homeTeam.idx, 2] <- totalHomeGames 
        
        # The total points scored in a home game;
        totalHomeGamePoints <- team.statistics[homeTeam.idx, 4] + match$homeGoals
        team.statistics[homeTeam.idx, 4] <- totalHomeGamePoints
      }
      
      # The total number of games each team has played in
      totalGames <- team.statistics[homeTeam.idx, 3] + 1
      team.statistics[homeTeam.idx, 3] <- totalGames 
    } 
    
    if(length(awayTeam.idx) != 0) {
      if(match$round == awayTeam) {
        # The number of HOME games each team has had
        totalHomeGames <- team.statistics[awayTeam.idx, 3] + 1
        team.statistics[awayTeam.idx, 2] <- totalHomeGames 
        
        # The total points scored in a home game;
        totalAwayGamePoints <- team.statistics[awayTeam.idx, 4] + match$awayGoals
        team.statistics[homeTeam.idx, 4] <- totalAwayGamePoints
      }
      
      # The total number of games each team has played in
      totalGames <- team.statistics[awayTeam.idx, 3] + 1
      team.statistics[awayTeam.idx, 3] <- totalGames 
    }
    
    
    # total number of points the team has scored
    totalPointsForMatch <- getTotalPoints(homeTeam, awayTeam, match$homeGoals, match$awayGoals, team.statistics[homeTeam.idx, 5], team.statistics[awayTeam.idx, 5], whoGotSnitch)
      team.statistics[homeTeam.idx, 5] <- totalPointsForMatch[1] 
      team.statistics[awayTeam.idx, 5] <- totalPointsForMatch[2] 

    # Provide these results in a data frame. 
...
# I've abstracted this function because point calculations are used more than once
